...
def ping(ip, retries, timeout):
    startupinfo = None
    if os.name == 'nt':  # Windowsの場合のみ
        startupinfo = subprocess.STARTUPINFO()
        startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
        startupinfo.wShowWindow = subprocess.SW_HIDE  # コマンドプロンプトのウィンドウを非表示にする

    for attempt in range(retries):
        try:
            command = ['ping', '-n', '1', '-w', str(timeout), ip] if os.name == 'nt' else ['ping', '-c', '1', '-W', str(timeout / 1000), ip]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for future in concurrent.futures.as_completed(results):
            ...
            if progress['value'] == progress['maximum']:
                progress.configure(style='Completed.Horizontal.TProgressbar')
...
style.configure('Completed.Horizontal.TProgressbar', background='green')
